...
### Passo 6: Usar o Secret na Aplicação
...
response = client.get_secret_value(SecretId='my-app-database-a1b2c3')
...
const secret = await client.getSecretValue({ 
  SecretId: 'my-app-database-a1b2c3' 
}).promise();
...
data "aws_secretsmanager_secret_version" "app_secret" {
  secret_id = module.app_secret.secret_arn
}

locals {
  secret_value = jsondecode(data.aws_secretsmanager_secret_version.app_secret.secret_string)
}
```

### Passo 7: Atualizar Valor do Secret Manualmente (Opcional)

Se você criou um secret sem geração automática de senha:

```bash
# Atualizar com string simples
aws secretsmanager put-secret-value \
  --secret-id my-manual-secret-a1b2c3 \
  --secret-string "minha-senha-super-secreta"
...
### Passo 8: Rotação de Secrets (Avançado)

Para configurar rotação automática:
